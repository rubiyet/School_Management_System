...
{
    public partial class AdminInformationInsertDeleteUpdate : Form
    {
        private DataAccess da { get; set; }
        private DataSet ds { get; set; }
        public AdminInformationInsertDeleteUpdate()
        {
            InitializeComponent();
            this.da = new DataAccess();

            string sql = "select * from Admin order by userId desc;";
        ...
        private void AdminInformationInsertDeleteUpdate_Load(object sender, EventArgs e)
        {
        ...
        private void GenerateAdminID()
        {
            try
            {
                ...
                string newId = "A" + (++number).ToString("d4");
        ...
        {
            string sql3 = "select userId from Admin where userId = '" + this.txtTeacherId.Text + "';";
            ...
            if (ds.Tables[0].Rows.Count == 0)
            {
                try
                {
                    string sql = @"insert into Admin 
                                values ('" + this.txtTeacherId.Text + "','" + this.txtFullName.Text + "', '" + this.txtEmail.Text + "', '" + this.cmbGender.Text + "', '" + this.dtpdob.Text + "', '" + this.txtNationality.Text + "', '" + this.txtReligion.Text + "', '" + this.txtBloodGroup.Text + "', '" + this.txtFatherName.Text + "', '" + this.txtMotherName.Text + "', '" + this.dtpJoingDate.Text + "', '" + this.dtpLeftDate.Text + "', '" + this.txtSalary.Text + "',  '" + this.txtQualification.Text + "' , '" + this.txtPresentAddress.Text + "', '" + this.txtParmanentAddress.Text + "', " + this.txtContactNumber.Text + ");";

                    int count = this.da.ExecuteUpdateQuery(sql);
                    if (count == 1)
                    {
                        label5.Text = ("Admin ID: " + this.txtTeacherId.Text + " Inserted Properly.");
                    }
                    else
                    {
                        
                    }
                    string sql2 = "select * from Admin order by userId desc;";
            ...
            {
                label5.Text = ("Admin ID: " + this.txtTeacherId.Text + " Already Insert.");
            }
        }
        ...
        private void btnDelete_Click_1(object sender, EventArgs e)
        {
            ...
            if (ds.Tables[0].Rows.Count == 1)
            {
                try
                {
                    ...
                    string sql = @"delete from Admin where userId = '" + teacher_id + "';";
                    int count = this.da.ExecuteUpdateQuery(sql);

                    if (count == 1)
                    {
                        label5.Text = ("Admin ID: " + this.txtTeacherId.Text + " has been deleted.");
                    }
                    else
                    {
        ...
        private void btnUpdate_Click_1(object sender, EventArgs e)
        {
            ...
            if (ds.Tables[0].Rows.Count == 1)
            {
                try
                {
                    ...
                    if (count == 1)
                    {
                        label5.Text = ("Admin ID: " + this.txtTeacherId.Text + " has been Updated.");
                    }
                    else
                    {
        ...
        private void txtAutoSearch_TextChanged_1(object sender, EventArgs e)
        {
            ...
            try
            {
                string sql = "select * from Admin where userId like '" + this.txtAutoSearch.Text + "%' order by userId desc;";
                this.PopulateGridView(sql);
            }
            catch (Exception ea)
            {
